...
export default function Application () {
    ...
    const onSubmit = async () => {
        try {
            ...
            if (error.message?.includes('CORS') || error.message?.includes('Network Error')) {
                errorMessage = 'Unable to connect to server. Please ensure the backend server is running on port 4011.';
                console.error('CORS/Network Error: Make sure to start the backend server with "node index.js" from the project root directory');
            ...
            setError({field: 'a', message: errorMessage})
        }finally{
            setSubmitting(false)
        }
    }
    ...
    const onUpdate = async () => {
        try {
            ...
            const {data} = await updateApplication(applicationId, updateData);
